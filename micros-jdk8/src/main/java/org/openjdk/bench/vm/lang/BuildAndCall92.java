...
//    final String v = "value";
  ...
  static String B(int count) {
    return    "import java.util.*; "
            ...
            + " "
            + " "
            + "   public Integer get( Map m, String k) { "
//            + "         System.out.println ( m + \" / \" + k);"
            + "         return (Integer) m.get(k) + 10;"
            + "   }"
            + " "
            + "   public Integer get2( Map m, String k) { "
//            + "         System.out.println ( m + \" / \" + k);"
            + "         return (Integer) m.get(k) + 20;"
            + "   }"
            + " "
            + "   public Integer get3( Map m, String k) { "
//            + "         System.out.println ( m + \" / \" + k);"
            + "         return (Integer) m.get(k) + 30;"
            + "   }"
            + " "
            + " "
            + "   public Integer get4( Map m, String k) { "
//            + "         System.out.println ( m + \" / \" + k);"
            + "         return (Integer) m.get(k) + 40;"
            + "   }"
            + " "
            + " "
            + "   public Integer get5( Map m, String k) { "
//            + "         System.out.println ( m + \" / \" + k);"
            + "         return (Integer) m.get(k) + 50;"
            + "   }"
            + " "
            + "   public Integer get6( Map m, String k) { "
//            + "         System.out.println ( m + \" / \" + k);"
  ...
  BuildAndCall92.BenchLoader loader1 = new BuildAndCall92.BenchLoader();
  ...
  public void setupClasses() throws Exception {
    ...
    for (index = 0; index < compiledClasses.length; index++) {
      ...
      IntStream.range(0, methodNames.length).forEach(m -> {
        try {
          methods[m] = c.getMethod(methodNames[m], java.util.Map.class, String.class);
        } catch (Exception e) {
...
//      System.out.println(m.getName());
        ...
        Integer result = (Integer) m.invoke(r,mapList.get(whichMap),k);
